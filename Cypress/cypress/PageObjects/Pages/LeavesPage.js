class LeavesPage {
  elements = {
    ...
    // Test case 11
    workWeekButton: () => cy.get('a.oxd-topbar-body-nav-tab-link').contains('Work Week'),
    mondayButton : () => cy.get('div.oxd-select-text.oxd-select-text--active'),
    halfDayOption: () => cy.get('.oxd-select-dropdown').contains('Half Day'),

    weekDaySaveButton: () => cy.get('button.oxd-button.oxd-button--medium.oxd-button--secondary.orangehrm-left-space').contains('Save'),
  ...
  // Test case 7 : Creating Holiday Succesfully
  leaveCreation() {
    cy.fixture('Data/HolidayForm').then((data) => {
  ...
  // Test Case 11 
  addWorkWeekFromLeave() {
    cy.wait(2000);
    this.elements.leaveTab().click();

    // Ahead on Configure tab
    this.elements.configureDropdown().click();
    // Click on the work Week button
    cy.wait(2000);
    this.elements.workWeekButton().click();

    this.elements.mondayButton().eq(1).click();
    this.elements.halfDayOption().click();


    // Save Button 

    this.elements.weekDaySaveButton().click();
